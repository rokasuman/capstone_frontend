...
const MyAppointment = () => {
  ...
  const [loading, setLoading] = useState(false) // Prevent double payments
  ...
  const handlePay = async (appointmentId) => {
    ...
    try {
      ...
      if (!clientSecret) throw new Error("Payment failed. No client secret.")
      ...
      if (paymentResult.error) {
        ...
        // 4. Update payment status in backend
        await axios.post(
  ...
  return (
    <div className="px-4">
      <h1 className='pb-3 mt-12 font-extrabold text-blue-700 text-center text-2xl'>My Appointment</h1>
      <div>
        {appointments.slice(0, 3).map((item, index) => (
          <div className="grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2" key={index}>
            ...
            <div className='flex-1 text-sm text-zinc-600'>
              ...
              <p className='text-sm'>
                <span className='font-semibold'>Date & Time:</span> {slotedDateFormat(item.slotDate)} | {item.slotTime}
              </p>
            </div>
